...
$("#submit-city").click(function(e) {
  ...
  $.ajax({
    url: `https://dev.virtualearth.net/REST/v1/Locations?countryRegion=US&adminDistrict=${
      inputArray[1]
    }&locality=${
      inputArray[0]
    }&key=AsNSrJ9nxY0hH82Hft6W7lSFK5zKER34LGJ3hYm70N6buBQXYlVneYV22zyYVLjL`
  }).then(
    data => {
      ...
      $.ajax({
        ...
        url: `https://cors-anywhere.herokuapp.com/https://www.brewerydb.com/browse/map/get-breweries?lat=${
          searchCoordinates[0]
        }&lng=${searchCoordinates[1]}&radius=15`
      }).then(
        data => {
          ...
          for (let i = 0; i < dataInfo.data.length; i++) {
            ...
            link.click(function(e) {
              ...
              if (storage.breweryWebsite !== null) {
                brewInfo.append(
                  `<a href="${storage.breweryWebsite}">${
                    storage.breweryWebsite
                  }</a>`
                );
              }
              ...
              favoriteButton.click(function(e) {
                ...
                // if breweryName isn't on the list
                let exists = $(`#favorites li:contains(${storage.breweryName})`).length; 
                  if(!exists){
                      favoritesList.append(
                    `<li><a href="${storage.breweryWebsite}" target="_blank">${
                      storage.breweryName
                    }</a></li>`
                  );
                }
